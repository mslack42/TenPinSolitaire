export function generateTotalsFromScoreBoard(scoreboard) {
    function getNextTwoBalls(scoreboard, afterRoundN) {
        const firstScoreboard =
            afterRoundN < 9 ? scoreboard[afterRoundN + 1] : []
        const secondScoreboard =
            afterRoundN < 8 ? scoreboard[afterRoundN + 2] : []

        return firstScoreboard
            .concat(secondScoreboard)
            .filter((b) => b !== null)
            .slice(0, 2)
    }
    ...
    let roundScores = scoreboard.reduce((totals, round, index) => {
        ...
        if (index == 9) {
            //Last frame
            if (
                strippedRound.length === 3 ||
                (strippedRound.length === 2 && roundSum < 10)
            ) {
                newTotal = roundSum
            }
        } else {
            if (roundSum == 10) {
                ...
                if (strippedRound.length == 1 && nextTwoBalls.length >= 2) {
                    ...
                    strippedRound.length == 2 &&
                    nextTwoBalls.length >= 1
                ) {
                    // Spare
