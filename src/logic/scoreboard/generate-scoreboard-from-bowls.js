export function generateScoreboardFromBowls(bowls) {
    const placedNumbers = bowls.reduce((scoreboard, bowl) => {
        ...
        if (
            latestRoundIndex === 9 &&
            ((latestRound.length == 2 &&
                latestRound[0] + latestRound[1] >= 10) ||
    ...
    const paddedForAllRounds = placedNumbers
        .concat(Array(10).fill([]))
        .slice(0, 10)
    const paddedEachRound = paddedForAllRounds.map((r, idx) =>
        r
            .concat(Array(idx === 9 ? 3 : 2).fill(null))
            .slice(0, idx === 9 ? 3 : 2)
    )
