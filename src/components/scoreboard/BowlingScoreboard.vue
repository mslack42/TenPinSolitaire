...
function displayBallFormat(roundIdx, ballIdx, scoreboard) {
    ...
    const previousBallScoreInRound =
        ballIdx > 0 ? scoreboard[roundIdx][ballIdx - 1] : null
    ...
    if (
        previousBallScoreInRound !== null &&
        ballScore + previousBallScoreInRound === 10
    ) {
        return '/'
    }
    if (ballScore === 0) {
...
<template>
    <table>
        <tr>
            <template v-for="i in ballsCount" :key="i">
                <th :colspan="ballBoxCounts[i - 1]">{{ i }}</th>
            </template>
        </tr>
        <tr>
            <template v-for="i in ballsCount" :key="i">
                <td v-for="j in ballBoxCounts[i - 1]" :key="j">
                    {{ displayBallFormat(i - 1, j - 1, scoreboard) }}
                </td>
            </template>
        </tr>
        <tr>
