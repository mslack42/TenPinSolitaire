...
export const usePinsStore = defineStore(storageKey, () => {
    ...
    const selectedPinCoords = computed(() =>
        getSelectedCardCoordinates(pins.value)
    )
    const remainingPinCoords = computed(() =>
        getRemainingCardCoordinates(pins.value)
    )
    const coordinatesHitThisBall = ref([])

    function updateSelectability() {
        pins.value = updatePinsSelectability(
            pins.value,
